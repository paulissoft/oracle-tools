...
is
  l_view_name all_views.view_name%type;
  l_view_owner all_views.owner%type;
  l_column_list varchar2(4000 byte) := null;
  l_column_tab dbms_sql.varchar2a;
  l_column_name all_tab_columns.column_name%type;
  l_found pls_integer;
  l_view_alias constant varchar2(2 byte) := 'VW';
  ...
  begin
    ...
    into    l_count_actual
    from    user_objects o
    where   ( o.object_type = 'VIEW' and o.object_name = l_view_name )
    ;
    if l_count_actual = p_count_expected
    then
      null;
      ...
      ( -20000
      , utl_lms.format_message
        ( 'Checking existence of view "%s". Expected count: %s; actual count: %s'
        , l_view_name
        , to_char(p_count_expected)
        , to_char(l_count_actual)
        )
...
begin
  case
    ...
    then
      -- 'abc' => 'ABC' and '"abc"' => 'abc'
      l_view_owner := trim('"' from oracle_tools.data_api_pkg.dbms_assert$enquote_name(p_view_owner, 'view owner'));
      l_view_name := trim('"' from oracle_tools.data_api_pkg.dbms_assert$enquote_name(p_view_name, 'view name'));

      -- check column list
      if p_column_list = '*'
      then
        l_column_list := l_view_alias || '.' || p_column_list;
      else
        oracle_tools.pkg_str_util.split
        ( p_str => p_column_list
        , p_delimiter => ','
...
' => 'ABC' and '"abc"' => 'abc'
          */
          ...
          begin
            ...
            into    l_found
            from    all_tab_columns c
            where   c.owner = l_view_owner
            and     c.table_name = l_view_name
            and     c.column_name = l_column_name;
          exception
            when no_data_found
            then
          ...
          l_column_tab(i_idx) := l_view_alias || '.' || '"' || l_column_name || '"';
        end loop;
      ...
      case upper(p_create_or_replace)
        when 'CREATE'
        then check_existence(0); -- VIEW SHOULD NOT EXIST
        when 'REPLACE'
        then check_existence(1); -- VIEW MUST EXIST
        else null;
      end case;

      execute_immediate
      ( 'CREATE OR REPLACE VIEW "' || l_view_name || '" AS ' ||
        'SELECT ' || l_column_list || ' FROM "' || l_view_owner || '"."' || l_view_name || '"' || ' ' || l_view_alias
      );
  end case;
end;
