...
is
  ...
  is
    with obj as
    ( select  /*+ materialize */
              t.object_schema() as owner
              /* scalar subquery to speed it up */
      ,       ( select t.dict_object_type() from dual ) as object_type
...
$end
  ...
  if p_recompile != 0
  then
    cfg_install_pkg.setup_session(p_plsql_warnings => p_plsql_warnings, p_plscope_settings => p_plscope_settings);
  end if;
  ...
  l_schema_object_filter.get_schema_objects(l_schema_object_tab);

$if oracle_tools.pkg_schema_object_filter.c_debugging $then
  dbug.print(dbug."info", '# schema objects: %s', l_schema_object_tab.count);
$end

  -- bulk fetch instead of loop because DDL is issued inside the loop which may impact the open cursor
  open c_obj(l_schema_object_tab, p_recompile);
...
begin
  ...
  ( select  lower(t.type) || ' ' || t.owner || '.' || t.name || ' ' ||
            ...
            t.text as text
    from    table
            ( oracle_tools.pkg_schema_object_filter.show_compiler_messages
              ( p_object_schema => p_object_schema
              , p_object_type => p_object_type
              , p_object_names => p_object_names 
              , p_object_names_include => p_object_names_include 
