...
begin
  if self is null or
     ...
     ( self.verb$ in ('ALTER', 'AUDIT', 'COMMENT', 'CREATE', 'DROP', 'GRANT', '--') and self.text_tab(1) like self.verb$ || ' %' )
  then
    null; -- OK
  else
    oracle_tools.pkg_ddl_error.raise_error
    ( oracle_tools.pkg_ddl_error.c_ddl_not_correct
    , 'First line should start with ALTER, AUDIT, COMMENT, CREATE, DROP, GRANT or --.'
