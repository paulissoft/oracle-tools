...
( session_id number default to_number(sys_context('USERENV', 'SESSIONID')) -- The session id (v$session.audsid)
, seq integer constraint ck$all_schema_objects$seq check (seq is not null and seq >= 1) -- Sequence within parent
, created timestamp(6) default sys_extract_utc(systimestamp) constraint ck$all_schema_objects$created check (created is not null)
...
, constraint pk$all_schema_objects primary key (session_id, seq)
)
organization index
tablespace data
...
create unique index uk$all_schema_objects on all_schema_objects (session_id, obj.id());
