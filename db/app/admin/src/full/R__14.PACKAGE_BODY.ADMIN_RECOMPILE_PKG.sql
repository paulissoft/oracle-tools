...
, p_exclude_oracle_maintained in pls_integer
)
...
is
  l_schema_tab constant sys.odcivarchar2list := get_schemas(p_schema);
begin
  if l_schema_tab.count = 0
  then
    raise no_data_found;
...
is
  l_schema_tab constant sys.odcivarchar2list := get_schemas(p_schema, case when p_exclude_oracle_maintained then 1 else 0 end);
begin
  if l_schema_tab.count = 0
  then
    raise no_data_found;
