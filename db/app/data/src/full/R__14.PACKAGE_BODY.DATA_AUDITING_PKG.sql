...
BEGIN
  ...
  THEN
    ...
    , P_DO_NO_SET_WHO => AUD$INS$WHO IS NOT NULL
    , P_DO_NO_SET_WHEN => AUD$INS$WHEN IS NOT NULL
    , P_DO_NO_SET_WHERE => AUD$INS$WHERE IS NOT NULL
    );
  ELSE
    ORACLE_TOOLS.DATA_AUDITING_PKG.SET_COLUMNS
    ...
    , P_DO_NO_SET_WHO => UPDATING('AUD$UPD$WHO')
    , P_DO_NO_SET_WHEN => UPDATING('AUD$UPD$WHEN')
    , P_DO_NO_SET_WHERE => UPDATING('AUD$UPD$WHERE')
    );
  END IF;
END;
>'
